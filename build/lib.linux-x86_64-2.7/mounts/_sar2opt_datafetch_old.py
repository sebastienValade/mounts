...
# --- set search options
# ingestiondate = "[2017-01-01T00:00:00.000Z TO 2017-06-01T00:00:00.000Z]"
ingestiondate = "[2017-05-01T00:00:00.000Z TO 2017-06-01T00:00:00.000Z]"
...
thresh_overlap_s1s2 = 100       # >> threshold overlap between s1 and s2 products (pair considered only if overlap >= threshold)
thresh_dt_s1s2 = 24 * 7         # >> thresholf time difference [hours] between between s1 and s2 products
...
thresh_overlap_poland = 100     # >> threshold overlap between poland and s2 scihub product (discards product if overlap < threshold)
thresh_area_s2 = 1.5            # >> threshold area s2 product [degrees?] (NB: wroclaw s2 = 1.54, poznan s2 = 1.60)
    ...
    for p2 in s2:
        ...
        if overlap_percentage >= thresh_overlap_s1s2:
            ...
            if dt.days * 86400 + dt.seconds < thresh_dt_s1s2 * 3600:
                print(print_space + 'overlap = ' + str(overlap_percentage))
                print(print_space + 't1 = ' + p1.metadata.beginposition)
                print(print_space + 't2 = ' + p2.metadata.beginposition)
                print(print_space + 'dt = ' + str(dt))
